...
def analyze():
    ...
    # Model that actually runs on THIS surface (PR→investigator, issue→issue
    # model), so the "Generated by AI (model: …)" footer is truthful. Computed
    # once so the early-exit artifacts below can't drift onto the wrong model.
...
def act():
    ...
    # Optional product attribution (e.g. "Reviewed by Shadow · <link>"). Empty
    # by default; scrubbed at load so it's a safe one-line markdown suffix.
    if cfg.bot_attribution:
        footer += f"\n*{cfg.bot_attribution}*"
...
# `**STATUS:** CONFIRMED`, `STATUS:CONFIRMED`, `STATUS: **CONFIRMED**`, so a
# bolded marker in the model output doesn't silently drop the finding.
...
def _scrub_ci_summary(text):
    """Neutralize comment delimiters + injection markers in third-party check-run
    names before they reach act()'s sanitize() — e.g. a matrix build named
    `Build (system: x86_64)` would otherwise trip the `system:` marker and null
    the clean response to ESCALATE. Uses a zero-width joiner (invisible, but
    breaks the substring match) rather than deletion so the name still reads
    right; re-checks after each sub to catch repeats."""
...
# id/verdict separator (e.g. `**VERDICT:** C1 | UPHELD`, `VERDICT: C1: UPHELD`),
# so a markdown-formatted verdict isn't silently parsed as None.
...
def _build_provenance_from_env():
    """Provenance for SKIP/ESCALATE paths with no Config in scope. Reporter AND
    issue must default to Haiku (_DEFAULT_REPORTER_MODEL), NOT BEDROCK_MODEL_ID,
    or setting only BEDROCK_MODEL_ID=Opus misattributes them to Opus while
    runtime ran Haiku. Mirrors Config's defaults from the same constants so a
    default bump can't drift this path."""
