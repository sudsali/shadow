...
class BedrockClient:
    def __init__(self, cfg):
        ...
        # Per-model breaker — Reporter (Haiku) and Investigator/Critic (Opus)
        # hit different throttle pools; a Haiku saturation should not block
    ...
    @property
    def available(self):
        # Coarse fallback for callers that don't know which model_id they'll
        # use; granular per-model checks should call is_model_available.
        return not self._open_models
    ...
    def invoke_with_usage(self, system_prompt, user_prompt, max_tokens=4096,
        ...
        if not self.is_model_available(effective_model):
            logger.warning("Circuit breaker open for %s, skipping", effective_model)
            return None, {}
        try:
            if self._guardrail_id:
                user_content = [{"guardContent": {"text": {"text": user_prompt}}}]
    ...
    def converse_with_tools(self, system_prompt, messages, tool_specs,
        ...
        tool_specs may be None to omit toolConfig, but callers that pass a
        message history containing prior toolUse/toolResult blocks MUST keep
        tool_specs populated — Bedrock validates history against toolConfig.
