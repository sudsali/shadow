...
Owns budget enforcement and runs a forced no-tool commit phase when the
tool budget is exhausted, so the model is physically constrained to emit
text instead of more tool calls.
"""
import logging
import time
...
def run(
    ...
    commit_phase_user_prompt (optional) drives a forced commit turn when
    the tool budget is exhausted; the prompt instructs the model to emit
    text only (toolConfig stays on the request — see _run_commit_phase).
    If unset, the loop terminates without a commit phase.
    ...
    for turn in range(max(0, caps.max_turns)):
        ...
        # Catch network/decode errors but let MemoryError, KeyboardInterrupt,
        # and SystemExit propagate — silencing those hides operator-relevant
        # signals as a generic "bedrock error".
...
def _build_user_content(trusted, untrusted, *, has_guardrail):
    """Diff (~200K) is too large for Bedrock's guardContent cap (~25K), so
    it stays in trusted; the prompt's <constraints> block defends those
    bytes. Untrusted title/body goes through guardContent for the
