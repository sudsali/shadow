    ...
    def _truncate_by_relevance(self, content, issue_text):
        ...
        for _, s in scored:
            ...
            if len(result) + len(chunk) > self._max_chars:
                break
            result += chunk
        # If the highest-scored section alone exceeds max_chars, the loop
        # breaks on the first iteration with result still "" — falling back
        # to a head-truncation keeps the model from running blind.
        if not result:
            return content[:self._max_chars]
        return result
